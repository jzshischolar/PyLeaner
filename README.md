...
```json
{
  ...
  "typeText": "Nat",
  "bodyText": "x + y",
  "bodyRange": {"start": {"line": 0, "character": 37}, "end": {"line": 0, "character": 42}},
  "fields": null,
...
For `structure` and structure-style `class` declarations, `fields` contains
one entry per directly declared field. Grouped binders such as `(x y : α)`
produce separate entries for `x` and `y` with the same source range.

| Field | Type | Description |
|-------|------|-------------|
| `name` | `string` | Source field name |
| `typeText` | `string` | Exact source type text; empty only for incomplete syntax without a type range |
| `binderKind` | `string` | One of: `explicit`, `implicit`, `instance` |
| `range` | `Range` | LSP range of the complete source binder |
| `projectionName` | `string \| null` | Fully qualified projection generated by Lean |
| `isClass` | `boolean \| null` | Whether the direct elaborated field type is a registered class (without opening binders internal to the field type) |
| `isProp` | `boolean \| null` | Whether the field is proof-valued (its elaborated declared type has sort `Prop`) |
| `isPropType` | `boolean \| null` | Whether the field's declared type reduces to `Prop` itself (for proposition-valued data such as `proof_goal : Prop`) |
| `className` | `string \| null` | Registered class name when `isClass` is true |

The five elaboration-derived values are `null` when the declaration is
incomplete or no projection was generated. Source fields are still returned,
so callers can report partial syntax without treating an elaboration error as
an RPC failure. `bodyRange` and `fields` are additive response keys; consumers
of the earlier `DeclarationInfo` shape can continue to ignore them.

`bodyRange` is an exact source slice for `bodyText`, including comments after
